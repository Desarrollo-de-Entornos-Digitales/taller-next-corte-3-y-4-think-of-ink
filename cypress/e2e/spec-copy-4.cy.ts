describe('Pruebas del Módulo de Nueva Publicación', () => {
    ...
    describe('Flujo de Creación y Formulario', () => {
        ...
        it('debería permitir crear un post básico de tipo texto de forma exitosa', () => {
            ...
            cy.get('input[placeholder="Escribe un título atractivo"]').type(
                'Flash Day este Sábado'
            );
            cy.get('textarea[placeholder="Cuéntales a todos sobre tu publicación..."]').type(
                'Tendremos diseños disponibles desde 100k.'
            );
        ...
        it('debería previsualizar y cargar una imagen correctamente', () => {
            cy.intercept('POST', '**/posts*', { statusCode: 201, body: {} }).as(
                'createPostMultipart'
            );

            cy.get('input[placeholder="Escribe un título atractivo"]').type(
                'Diseño Neo Tradicional'
            );
            cy.get('textarea[placeholder="Cuéntales a todos sobre tu publicación..."]').type(
...
//No pude corregirlo, sin importar que intentara
